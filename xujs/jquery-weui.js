...
+function ($) {
  ...
  PTR.prototype.touchEnd = function() {
    ...
    if(Math.abs(this.diffY) <= this.distance) {
      ...
      this.container.trigger("pull-to-refresh");//添加样式 在 jQuery-weui-css 里面
    }
  };
...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      var defaults = {
          ...
          rotateEffect: false,//是否启用3D效果   显示在弹出框的手机类型 这一部分
          momentumRatio: 7,
          freeMode: false,
          // Common settings
          scrollToInput: true,
          inputReadOnly: true,//是否会在input上添加一个readOnly的属性  readonly:表示只读(只能看到,不能修改)的输入域(框)
          toolbar: true,//是否显示工具栏  相当于页面中的  您的姓名 这一部分 
          toolbarCloseText: '完成',//关闭按钮的文字
          title: '请选择',
...
+ function($) {
  ...
  defaults = $.fn.select.prototype.defaults = {
    ...
    onChange: undefined, //function  当选择值改变的时候出发
    beforeClose: undefined, // function 关闭之前，如果返回false则阻止关闭
    onClose: undefined, //function   当picker被关闭时出发
...
+function ($) {
  ...
  defaults = $.fn.calendar.prototype.defaults = {
    ...
    touchMove: true,
    animate: true,
    //closeOnSelect：用户选择一个时间后就自动关闭   
    //true：当选择第一个日期之后会自动跳转到最后一个日期  false:不会跳转
    closeOnSelect: true,
    monthPicker: true,//是否显示月份
    monthPickerTemplate: 
    ...
    toolbarTemplate:  //Toolbar 的HTML结构
        '<div class="toolbar">' +
            '<div class="toolbar-inner">' +
                '{{yearPicker}}' +
                '{{monthPicker}}' +
